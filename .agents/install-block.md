...
Then, from inside a session (or `claude plugins install` from the CLI):
...
The plugin is Claude Code only. Everywhere else, [skills.sh](https://skills.sh/danceiny/skills) copies editable skill files into the project. Use the whole-set form on `README.md`:
...
```bash
npx skills@latest add danceiny/skills --skill=<name>
```
...
`.claude-plugin/marketplace.json` makes the repo its own single-plugin marketplace (`/plugin marketplace add danceiny/skills`, then `/plugin install danceiny-skills@danceiny`). This is the primary install route for this repo.
