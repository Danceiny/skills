...
Because these pages are published on `aihero.dev`, **every link is absolute**: never a repo-relative path. A link to another skill points at `https://aihero.dev/skills-<name>`; a link into the repo points at its full `https://github.com/danceiny/skills/...` URL. A relative link that works in the repo breaks once published.
...
- **This repo's issues.** `gh issue list --repo danceiny/skills --search "<skill-name>" --state all`. A question filed twice is a question the page owes an answer to.
...
- **The map.** Point to [ask-skills](https://aihero.dev/skills-ask-skills), the router over the whole set, so this page stays a node and never has to redraw the graph.
...
- Every `docs/` link is repo-relative and resolves on GitHub; external links are absolute.
